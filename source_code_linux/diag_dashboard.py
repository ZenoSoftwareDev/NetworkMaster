...
def get_adapters_data():
    """Pobiera statystyki bezpośrednio z /sys/class/net/ dla systemu Linux"""
    data = {}
    try:
        ifaces = os.listdir('/sys/class/net/')
        for iface in ifaces:
            if iface == 'lo': 
                continue # Omijamy pętlę zwrotną
            
            data[iface] = {'rx': 0, 'tx': 0, 'ip': 'Brak IP'}
            ...
            # Pobieranie liczników RX/TX z systemu plików
            try:
...
def run():
    ...
    while True:
        try:
            ...
            if p == -1: ping_str = f"{R}BRAK INTERNETU (Timeout){RESET}"
            elif p < 40: ping_str = f"{G}{p} ms (Znakomity){RESET}"
            elif p < 100: ping_str = f"{Y}{p} ms (Średni){RESET}"
            else: ping_str = f"{R}{p} ms (Wysoki){RESET}"
            ...
            for name in selected_adapters:
                ...
                ip = info.get('ip', 'Brak IP (Rozłączono)')
