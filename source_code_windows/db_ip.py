...
def select_profile_menu(mode="SELECT"):
    while True:
        ...
        header = f" {'ID':<3} | {'NAZWA':<12} | {'IP':<15} | {'MASKA':<15} | {'BRAMA':<15} | {'DNS 1':<15} | {'DNS 2'}"
        ...
        for i, p in enumerate(profiles, 1):
            ...
            print(f" {i:<3} | {n:<12} | {p[1]:<15} | {p[2]:<15} | {p[3]:<15} | {p[4]:<15} | {p[5]}")

        print("-" * len(header))
        
        # --- ROZRÓŻNIENIE MENU (ZALEŻNIE OD TRYBU WEJŚCIA) ---
        if mode == "SELECT":
            # Wchodzimy z karty sieciowej -> Potrzebujemy [ID] do wyboru
            print(f" {G}[ID]{RESET} Wybierz   {G}[A]{RESET} Dodaj   {G}[E]{RESET} Edytuj   {G}[D]{RESET} Usuń   {R}[0]{RESET} Powrót")
        else:
            # Wchodzimy z Main Menu -> Tylko zarządzanie, bez [ID]
        ...
        # Logika wyboru numeru działa TYLKO w trybie SELECT
        if mode == "SELECT":
            try:
                idx = int(c)-1
...
def run_management():
    # To wywołuje main.py -> Ustawiamy tryb MANAGE (bez wyboru ID)
...
def manual_input_profile(old_data=None):
    ...
    print(" [1] 255.255.255.0  [2] 255.255.0.0  [3] 255.0.0.0  [4] WŁASNA")
